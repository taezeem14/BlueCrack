...
Supports a Flask Web UI (default) and a full-featured CLI with interactive wizard.

Usage:
    python bluecrack.py              # Launch Web UI (default)
    python bluecrack.py --web        # Launch Web UI explicitly
    python bluecrack.py -i           # Interactive CLI wizard
    python bluecrack.py --url ...    # Direct CLI mode
"""
...
# ── Graceful stop flag (shared between CLI threads) ──
...
parser.add_argument(
    "--web", action="store_true", help="launch the Flask web UI"
)
parser.add_argument(
    "--gui", action="store_true", help="launch the Flask web UI (alias for --web)"
)
...
parser.add_argument(
    ...
    type=int,
    default=5000,
    help="web UI port number (default: 5000)",
)
...
def _has_cli_args() -> bool:
    """Check if the user provided any CLI-mode arguments."""
    return bool(
        args.url
        or args.username
        or args.userfile
...
if args.web or args.gui or (len(sys.argv) == 1) or (not _has_cli_args() and not args.interactive):
    # Launch Flask Web UI
    try:
        from app import run_server
        run_server(host=args.host, port=args.port)
    except ImportError as e:
        print(f"{_RED}[-] Flask not installed. Run: pip install flask flask-socketio{_RESET}")
